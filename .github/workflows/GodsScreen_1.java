...
public class GodsScreen extends Screen {
    ...
    private static final int TITLE_MARGIN_LEFT = 80; // Adjusted to move title left
    private static final int TITLE_MARGIN_TOP = 20; // Adjusted for vertical position
    ...
    protected void init() {
        ...
        int bottomButtonY = topButtonY + BUTTON_HEIGHT + MARGIN_BOTTOM; // Position for the bottom row
        ...
        // Add the "Return" button at the top-right corner
        int returnButtonX = width - RETURN_BUTTON_WIDTH - MARGIN_RIGHT; // Right-aligned
        int returnButtonY = MARGIN_TOP; // Top-aligned
        this.addDrawableChild(ButtonWidget.builder(Text.of("Return"), btn -> {
            if (this.client != null) {
                this.client.setScreen(new LoreScreen(Text.of("Lore"))); // Go back to LoreScreen
            }
