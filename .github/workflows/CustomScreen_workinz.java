...
public class CustomScreen extends Screen {
    ...
    public void render(DrawContext contents, int mouseX, int mouseY, float delta) {
        ...
        // Draw centered text
        contents.drawCenteredTextWithShadow(textRenderer, Text.literal("You must see me"), width / 2, height / 2, 0xFFFFFFFF); // Fully opaque white text

        // Draw a small white square
        contents.fill(10, 10, 20, 20, 0xFFFFFFFF); // Fully opaque white

        // Draw a border on the right side of the screen with a 10-pixel margin
        int borderX = width - 110; // 10-pixel margin from the right edge
        contents.drawBorder(borderX, 10, 100, height - 20, 0xFF00FF00); // Green border filling the right side
