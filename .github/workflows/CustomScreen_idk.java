...
public class CustomScreen extends Screen {
    ...
    @Override
    protected void init() {
        // Load race names from the JSON file
        List<JsonReader.RaceData> raceButtonsData = JsonReader.readJson("raceSelectButtons.json");
        ...
        if (raceButtonsData != null && !raceButtonsData.isEmpty()) {
            List<String> raceNames = (List<String>) raceButtonsData.get(0); // Use the first array for button labels

            for (int row = 0; row < GRID_ROWS; row++) {
                for (int col = 0; col < GRID_COLUMNS; col++) {
                    int index = row * GRID_COLUMNS + col;
                    if (index < raceNames.size()) { // Check if index is within bounds
                        ...
                        String buttonText = raceNames.get(index);
